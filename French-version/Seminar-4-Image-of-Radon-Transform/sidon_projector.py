...
@nb.njit
def radon2d_sidon(image, ltheta, lshift, radius=1.0):

    """
        ltheta - list of angles
        lshift - list of shifts
        
        hence, data = ltheta x lshift
    """
    ntheta = len(ltheta)
    nshift = len(lshift)
    ...
    for i_theta in range(ntheta):
        for i_shift in range(nshift):
            proj[i_theta][i_shift] = sidon_line_projector(image, ltheta[i_theta], 
                                                          lshift[i_shift], radius)    
    return proj